...
module.exports = {
    ...
    async execute(client, message,cmd,args,Discord){
        ...
        var sorted = allPlayerData.sort((a, b) => (b.raidsWon) - (a.raidsWon));
        ...
        for (let j = 0; j < sorted.length; j++){
            ...
            newEmbed.addFields(
                { name: `#${Number(pos) + Number(pageNumber) * 10}: ${user.username}#${user.discriminator}`, value: `All-time Raids Won: ${new Intl.NumberFormat().format(sorted[j].raidsWon)}\nWeekly Raids Won: ${new Intl.NumberFormat().format(sorted[j].weeklyRaidsWon)}`}
            )
